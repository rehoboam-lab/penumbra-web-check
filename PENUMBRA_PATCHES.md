...
| 5 | `server.js`: new `GET /healthz` route, mounted outside the `/api` rate limiters, which invokes the real `/api/dns` handler internally against `HEALTHZ_TARGET_URL`. | With `API_ENABLE_RATE_LIMIT=true`, Render's ~5s health probe against `/api/dns?url=example.com` exhausted the tightest per-IP limit (100 req/10 min) after ~8.3 minutes, so the health check returned `429`, Render killed the instance as unhealthy, and the fresh instance reset the in-memory counter — a permanent ~9-minute restart loop (observed continuously 2026-07-25 → 2026-08-03, ~160 restarts/day). The probe must be limiter-exempt, but still exercises the API layer so a wedged service fails the probe. Render `healthCheckPath` must be switched to `/healthz`. | `HEALTHZ_TARGET_URL` (default `example.com`) |
